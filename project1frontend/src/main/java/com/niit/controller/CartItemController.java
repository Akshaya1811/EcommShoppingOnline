...
public class CartItemController {
	...
	public String removeCartItem(@PathVariable int cartItemId) {
		...
		// select * from cartitem where user_email=? - to execute the query redirect
		// /cart/getcart
	}
	...
	public String showpaymentpage(Model m) {
		// String Username="gayu";
		// List<CartItem> cartItems=cartItemDao.getCart(Username);
		// m.addAttribute("grandTotal",this.calcTotalAmount(ListCartItem));
		// m.addAttribute("Address",CustomerDao getusername(username)getaddress());
		return "payment";
	}
	// display receipt page//

	/*
	 * @RequestMapping(value = "/all/OrderConfrim"),method=RequestMethod.POST)
	 * public String showreceiptpage(Model m) { //System.out.println("tests");
	 * return "OrderConfrim"; }
	 */

	/*
	 * @RequestMapping(value = "/cart/Invoice") public String invoice( Model model,
	 * 
	 * @AuthenticationPrincipal Principal principal, HttpSession session) { String
	 * email = principal.getName(); User user = cartItemDao.getUser(email);
	 * 
	 * Customer customer = user.getCustomer();
	 * //customer.setShippingaddress(shippingaddress);// customer.shippingaddress ->
	 * updated shippingaddress obj customer.setUser(user);
	 * //1model.addAttribute("shippingaddress",customer.getShippingaddress());//this
	 * is to disp ship add user.setCustomer(customer);// user.customer -> updated
	 * customer references
	 * 
	 * List<CartItem> cartItems = cartItemDao.getCart(email);
	 * 
	 * for (CartItem cartItem : cartItems) { Product product =
	 * cartItem.getProduct(); if ((product.getQuantity() - cartItem.getQuantity()) <
	 * 0) { cartItemDao.removeCartItem(cartItem.getCartItemId());
	 * model.addAttribute("productNA", product); return "productnotavailable"; } }
	 * 
	 * // Calculate grandTotal
	 * 
	 * double grandTotal = 0; for (CartItem cartItem : cartItems) {// for(T
	 * o:collection) grandTotal = grandTotal + cartItem.getTotalPrice(); }
	 * 
	 * // create CustomerOrder object CustomerOrder customerOrder = new
	 * CustomerOrder(); customerOrder.setPurchaseDate(new Date());
	 * customerOrder.setUser(user);// customerOrder -> user -> customer ->
	 * shippingaddress customerOrder.setGrandTotal(grandTotal); customerOrder =
	 * cartItemDao.createCustomerOrder(customerOrder);//2 if (cartItems.size() > 0)
	 * customerOrder = cartItemDao.createCustomerOrder(customerOrder);
	 * 
	 * // Remove all cartitems from the cartItem table // decrement the quantity of
	 * the product // add customerorder and List<CartItem> to an model attributes
	 * and return // "orderdetails"
	 * 
	 * for (CartItem cartItem : cartItems) { Product product =
	 * cartItem.getProduct(); // In stock =30 // requested quantity=20 // update
	 * product quantity=30-20 where id=? product.setQuantity(product.getQuantity() -
	 * cartItem.getQuantity());// decrement the product quantity
	 * productDao.updateProduct(product);// update product set quantity=7 where
	 * id=38 cartItemDao.removeCartItem(cartItem.getCartItemId()); }
	 * model.addAttribute("customerorder", customerOrder);//
	 * order=[orderId,purchaseDate,grandTotal,User] model.addAttribute("cartItems",
	 * cartItems); session.setAttribute("cartSize", 0); return "orderDetails";//
	 * orderDetails }
	...
	@RequestMapping(value = "/all/Thanks")
	public String thanksgivingpg(Model m) {
		// String Username="gayu";
		// List<CartItem> cartItems=cartItemDao.getCart(Username);
		// m.addAttribute("grandTotal",this.calcTotalAmount(ListCartItem));
		// m.addAttribute("Address",CustomerDao getusername(username)getaddress());
